// src/lib/filmesService.ts

// Definindo o tipo de um Filme
export interface Filme {
  ...
  export function adicionarFilme(filme: Omit<Filme, 'id'>): void {
    const filmes = obterFilmes();
    const novoFilme: Filme = { ...filme, id: Date.now() }; // Gerando um ID único
    filmes.push(novoFilme);
    salvarFilmes(filmes);
  }
  
  export function atualizarFilme(id: number, dadosAtualizados: Partial<Filme>): void {
    const filmes = obterFilmes();
